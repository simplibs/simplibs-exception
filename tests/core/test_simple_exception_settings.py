...
from simplibs.exception.core._helpers.internal_exceptions.SimpleExceptionSettingsError import (
...
def test_system_blacklist_contains_expected_default_patterns():
    ...
    assert "simplibs/" in SimpleExceptionSettings._SYSTEM_BLACKLIST
...
def test_reset_location_blacklist_consistency():
    """Validates that factory reset restores user-defined blacklist back to system default."""
    SimpleExceptionSettings.LOCATION_BLACKLIST = ("something.py",)
    SimpleExceptionSettings.reset()
    assert SimpleExceptionSettings.LOCATION_BLACKLIST == SimpleExceptionSettings._SYSTEM_BLACKLIST