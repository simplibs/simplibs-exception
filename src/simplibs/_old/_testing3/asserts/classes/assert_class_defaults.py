...
By querying the class dictionary, the function determines what the developer *explicitly* configured:
- **Field exists in `__dict__`:** The raw declared value is fetched and routed for active validation.
- **Field is missing:** Falls back to the `UNSET` sentinel token, instructing the downstream field 
  verification blade (`assert_exception_fields`) to safely bypass checking for that property.
