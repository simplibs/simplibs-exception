...
class Params(Sequence[Any]):
    ...
    _values: tuple[Any, ...]
    _args_and_kwargs: tuple[tuple[Any, ...], dict[str, Any]]
    ...
    @property
    def args_and_kwargs(self) -> tuple[tuple[Any, ...], dict[str, Any]]:
        if self._values and isinstance(self._values[-1], Kwargs):
            args = self._values[:-1]
            kwargs = dict(self._values[-1])
        else:
            args = self._values
            kwargs = {}
        # noinspection PyTypeChecker
...
test runners. It explicitly informs parameter evaluation pipelines (`process_params`) that the encapsulated 
