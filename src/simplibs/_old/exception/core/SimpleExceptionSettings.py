...
# Inners
from ._settings_meta import SimpleExceptionSettingsMeta
...
class SimpleExceptionSettings(metaclass=SimpleExceptionSettingsMeta):
    ...
    DEFAULT_LOCATION_BLACKLIST: tuple[str, ...] = ("simplibs/exception",)
    DEFAULT_MESSAGE_MODE: ModeBase = PRETTY
    DEFAULT_VALUE_TRUNCATION_LENGTH: int = 70
    ...
    def reset(cls):
        ...
        cls.DEFAULT_LOCATION_BLACKLIST = ("simplibs/exception",)
        cls.DEFAULT_VALUE_TRUNCATION_LENGTH = 70
...
# skips all frames whose path contains "simplibs/exception"
```
...
### DEFAULT_VALUE_TRUNCATION_LENGTH
An integer defining the maximum number of characters shown for a value in the 
exception message.

| Value | Behaviour |
|-------|-----------|
| `int` | Maximum length of `repr(value)` before truncation (default: 100) |

If the value representation exceeds this limit, it is truncated and appended 
with a count of omitted characters (e.g., `[truncated, 452 chars]`). This 
prevents large data structures from overwhelming the terminal output.
