...
# Outers
from ..._helpers import compare_strings, maybe_subtest
...
It decouples the extraction and instantiation mechanics from property verification, allowing it 
to serve as an atomic brick across diverse composition boundaries.

## Composition & Reusability Matrix
This routine is intentionally agnostic to *how* the exception was obtained, enabling three core operational modes:
1. **Direct Standalone Verification:** Invoked by end-users wishing to audit properties on an already caught exception instance.
2. **Class Facade Assembly:** Leveraged by `assert_exception_class` engines to verify default metadata properties populated during out-of-the-box standard initialization tests.
3. **Functional Facade Assembly:** Leveraged by `assert_exception_function` chains to deep-check operational parameters on errors raised dynamically during functional boundary execution passes.

## Sentinel Protection Design
By defaulting fields to the `UNSET` token rather than standard `None`, the function distinguishes between:
- **`UNSET`:** Bypasses evaluation entirely (the developer does not look at or care about this specific property).
- **`None` / Empty state:** Actively asserts that the exception property is explicitly empty or deactivated.
"""