...
def print_intercepted_exception(
    ...
    *,
    ...
    """
    Renders information exclusively about a caught exception instance supporting standard multi-line, flat oneline, or logfmt formatting.

    If the input value is empty or not an Exception instance, the function gracefully returns None.

    Output Formats:
        Standard Mode (Human-friendly multiline breakdown):
            Structure: <PREFIX> (<EXCEPTION_TYPE>):\n    <EXCEPTION_MESSAGE>
            Example:   Intercepted exception (ValueError):
                           Expecting value: line 1 column 1 (char 0)

        Oneline Mode (_oneline=True):
            Structure: <PREFIX> (<EXCEPTION_TYPE>): <EXCEPTION_MESSAGE_FLATTENED>
            Example:   Intercepted exception (ValueError): Expecting value: line 1 column 1 (char 0)

        Log Mode (_log_mode=True):
            Structure: intercepted_exception='<EXCEPTION_TYPE>'
            Example:   intercepted_exception='ValueError'
    """
